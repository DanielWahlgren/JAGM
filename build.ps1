...
task DebugBuild -if ($Configuration -eq "debug") {
    ...
    if(Test-Path ".\Output\temp\$($ModuleName)\$($ModuleVersion)"){
        ...
        Remove-Item ".\Output\temp\$($ModuleName)\$($ModuleVersion)" -Recurse -Force | Out-null 
    }

    Write-Verbose -Message "Creating new temp module version folder: .\Output\temp\$($ModuleName)\$($ModuleVersion)."
    try {
        New-Item -Path ".\Output\temp\$($ModuleName)\$($ModuleVersion)" -ItemType Directory | Out-null 
    }
    catch {
    ...
    try {
        ...
        New-Item -Path ".\Output\temp\$($ModuleName)\$ModuleVersion\$($ModuleName).psm1" -ItemType File | Out-null 
    }
    catch {
